@tool class_name Multiline2D extends Node2D
...
func _draw() -> void:
	if improved:
		CanvasItemFuncs.draw_multiline(self, points, Color.WHITE, width,
				anti_aliasing, anti_aliasing_size)
	else:
		draw_multiline(points, Color.WHITE, width, anti_aliasing)
